...
const pets = [{ name: "Cheese" }, { name: "Pepe" }];

function index() {
  return (
    <>
      <main className="main mx-10">
        <h3 className="flex text-center justify-between">
          {pets.map((e) => (
            <ul className="bg-white text-black w-[50%]" key={e.name}>
              {e.name}
            </ul>
          ))}
        </h3>
        ...
        <ul className="list-disc">
          <button onClick={() => console.log("You clicked horray")}>
            Button
          </button>
